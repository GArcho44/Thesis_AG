...
pfunc.1 = function(data, x, y, s, col_var, color_vec, t, drop=TRUE) {
    ggplot(data, aes_string(x, y, size = s, col = col_var)) +  geom_point(alpha = 0.8, show.legend = TRUE) +   scale_size_continuous(limits=c(0,18000),breaks=seq(0, 18000, 1000),range=c(0,40)) + scale_colour_manual(values=color_vec, drop=drop) + theme(axis.text.x = element_text(size=20, angle=90, vjust = 0.5), plot.title = element_text(hjust = 0.5, size=30), axis.text.y = element_text(size=20), plot.margin=unit(c(3,0.1,3,0.1), 'cm')) + rremove("xlab") + rremove("ylab") + labs(title = t)
...
figure.1 <- ggarrange(plotlist = plot.list, 
                      ncol = 4, nrow = 3, align = "hv", common.legend = TRUE, legend = FALSE, 
                      labels = NULL, 
...
# Annotate figure
annotate_figure(figure.1, left = textGrob("Bacterial species", rot = 90, vjust = 1, gp = gpar(cex = 1.8)), bottom = textGrob("Days since birth", gp = gpar(cex = 1.8)))
```
