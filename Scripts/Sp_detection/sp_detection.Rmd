...
# # Also filter them based on the specific cut-off calculated.
# 
# # Introduce an index help number i
# i <- 1
# 
# # Creat a list to store the cut-off filtered samples
# cut_off_filtered <- list()
# 
# # Loop through every .tsv file (for each individual)
# for (data in id.list)
# {
...
#   # Make a summary of the data
#   n <- length(id.list[[i]])
#   id.list[[i]] <- full_join(detection1,detection2,by=colnames(detection1)) %>% full_join(detection3, by=colnames(detection1)) %>% pivot_longer(cols=2:all_of(n), names_to = 'Samples', values_to = 'Counts')
# 
#   ## Filter the samples based on the cut_off of reads: N = 1018 (reads per sample) ##
# 
#   # Make another table of the data
#   cut_off_filtered_tmp <- full_join(detection1,detection2,by=colnames(detection1)) %>% full_join(detection3, by=colnames(detection1))
# 
#   # Transpose the data frame
#   # first remember the names
#   names <- cut_off_filtered_tmp$Bacterial_species
#   cut_off_filtered_tmp <- as.data.frame(t(cut_off_filtered_tmp[,-1]))
#   colnames(cut_off_filtered_tmp) <- names
# 
#   # Replace NA values with 0
#   id.list[[i]][is.na(id.list[[i]])] <- 0
#   cut_off_filtered_tmp[is.na(cut_off_filtered_tmp)] <- 0
# 
#   # Filter the samples
#   cut_off_filtered_tmp <- cut_off_filtered_tmp %>% filter(`Bifidobacterium longum`>= 1018 | `Escherichia coli` >= 1018 | `Bacteroides dorei/vulgatus` >= 1018)
# 
#   # Append the temporary data to the cut_off_filtered list
#   cut_off_filtered <- append(cut_off_filtered, list(cut_off_filtered_tmp))
# 
#   ## <--> ##
# 
#   # Make a factor variable from the continues counts variable
#   id.list[[i]]$Counts.factor <- cut(id.list[[i]]$Counts, c(0,1000,2000,3000,4000,5000,6000))
#   id.list[[i]] <- tidyr::unite(id.list[[i]],"Bac_counts",Bacterial_species,Counts.factor,remove = F)
# 
#    # Replace the sample names with corresponding day
#   id.list[[i]]$Samples <- gsub("ID...d","",id.list[[i]]$Samples) # comment these lines if you want samples instead of days on the plot
#   id.list[[i]]$Samples <- gsub(".s...","",id.list[[i]]$Samples)
#   id.list[[i]]$Samples <- as.numeric(id.list[[i]]$Samples)
# 
#   # Change the variable name depending on the .tsv file
#   assign(paste0("full_report", i), id.list[[i]])
# 
#   # Increase the value of the index help number i
#   i <- i + 1
# }
# 
# # Write the filtered files
# write.xlsx(cut_off_filtered[[1]], file = "data/ID1_report.xlsx", rowNames=TRUE)
# write.xlsx(cut_off_filtered[[2]], file = "data/ID2_report.xlsx", rowNames=TRUE)
# write.xlsx(cut_off_filtered[[3]], file = "data/ID3_report.xlsx", rowNames=TRUE)
